...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch all users
        const usersResponse = await axios.get("/api/auth/all-users");
        setUsers(usersResponse.data.users);

        // Fetch all blogs
        const blogsResponse = await axios.get("/api/blog/all-blog");
        setBlogs(blogsResponse.data.blogs);

        // Fetch all comments
        const commentsResponse = await axios.get("/api/blog/all-comments");
      ...
      <div className="w-full flex flex-col gap-8 lg:flex-row">
        ...
        <div className="lg:w-[85%] flex flex-col gap-6">
          ...
          <h1 className="text-2xl font-semibold mb-4 leading-9">
            Welcome to your dashboard Prabhat!
          </h1>
